-- ============================================================================
-- MIGRATION : Système de Finale Ultra Personnalisable
...
ALTER TABLE public.finals
  ADD COLUMN IF NOT EXISTS mode TEXT DEFAULT 'elimination'
    CHECK (mode IN ('elimination', 'lives', 'points_race', 'sudden_death', 'tournament'));
...
-- Policies pour final_lives
DROP POLICY IF EXISTS "Final lives visible by authenticated users" ON public.final_lives;
CREATE POLICY "Final lives visible by authenticated users"
  ON public.final_lives FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "Authenticated users can manage final lives" ON public.final_lives;
CREATE POLICY "Authenticated users can manage final lives"
  ON public.final_lives FOR ALL TO authenticated
  USING (true) WITH CHECK (true);

-- Policies pour joker_usages
DROP POLICY IF EXISTS "Joker usages visible by authenticated users" ON public.joker_usages;
CREATE POLICY "Joker usages visible by authenticated users"
  ON public.joker_usages FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "Authenticated users can insert joker usages" ON public.joker_usages;
